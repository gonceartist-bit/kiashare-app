(function () {
  ...
  // STUN for direct connections when possible, plus a public TURN relay as a
  // fallback for topologies (like a phone hotspot + PC) where the two devices
  // can't reach each other directly. Confirmed needed for this setup — do not
  // remove without testing on the actual hotspot topology first.
  const ICE_CONFIG = {
    iceServers: [
  ...
  function startSenderPeer(retry) {
    if (typeof Peer === "undefined") {
      qrStatus.textContent = "کتابخانه‌ی اتصال لود نشد (peerjs.min.js). صفحه رو رفرش کن یا اتصال اینترنت رو چک کن.";
      return;
    }
  ...
  function connectToCode(code) {
    if (typeof Peer === "undefined") {
      connectStatus.textContent = "کتابخانه‌ی اتصال لود نشد (peerjs.min.js). صفحه رو رفرش کن یا اتصال اینترنت رو چک کن.";
      return;
    }
